    ...
    try {
      ...
      return (
        data.items?.map((item: any) => ({
          ...
          isbn: item.volumeInfo.industryIdentifiers?.find((id: any) => id.type === 'ISBN_13')
            ?.identifier,
  ...
  static async searchBooks(query: string): Promise<MetadataSearchResult[]> {
    try {
      const response = await fetch(
        `${this.BASE_URL}/search.json?q=${encodeURIComponent(query)}&limit=10`,
      );
      const data = await response.json();
      ...
      const mockResults: MetadataSearchResult[] = [
        {
          ...
          description:
            'This is a demo movie result. In production, this would use the actual TMDB API.',
          genre: ['Action', 'Drama'],
          rating: 8.5,
          source: 'tmdb',
        },
...
export class MusicBrainzAPI {
  ...
  static async searchMusic(query: string): Promise<MetadataSearchResult[]> {
    try {
      ...
      return (
        data['release-groups']?.map((release: any) => ({
          id: release.id,
          title: release.title,
