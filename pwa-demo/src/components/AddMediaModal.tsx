...
const AddMediaModal: React.FC<AddMediaModalProps> = ({ onClose, onAdd }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-light-navy w-full max-w-md rounded-lg shadow-xl flex flex-col"
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <label htmlFor="mediaType" className="block text-sm font-medium text-light-slate">
              Media Type
            </label>
            <select
              id="mediaType"
            ...
            >
              {(Object.keys(MediaType) as Array<keyof typeof MediaType>).map((key) => (
                <option key={key} value={MediaType[key]}>
                  {MediaType[key]}
                </option>
              ))}
            </select>
          ...
          <div>
            <label htmlFor="creator" className="block text-sm font-medium text-light-slate">
              {getCreatorLabel(mediaType)}
            </label>
            <input
              type="text"
              id="creator"
