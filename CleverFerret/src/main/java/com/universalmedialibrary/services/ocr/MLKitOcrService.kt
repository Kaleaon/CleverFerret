            ...
            val lines = block.lines.map { line ->
                ...
                val words = line.elements.map { element ->
                    ...
                    TextWord(
                        ...
                        confidence = OcrService.DEFAULT_CONFIDENCE // ML Kit doesn't provide per-element confidence
                    )
                }
                TextLine(
                    text = line.text,
                    boundingBox = lineRect,
                    confidence = OcrService.DEFAULT_CONFIDENCE, // ML Kit doesn't provide per-line confidence
                    words = words
                )
            }
            TextBlock(
                ...
                confidence = OcrService.DEFAULT_CONFIDENCE, // ML Kit doesn't provide per-block confidence
                lines = lines
            )
        }
