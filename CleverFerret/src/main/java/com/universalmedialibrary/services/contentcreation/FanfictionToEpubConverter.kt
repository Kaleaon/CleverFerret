...
class FanfictionToEpubConverter @Inject constructor(
    ...
    private val updateManager: StoryUpdateManager
) {
    ...
    /**
     * Convert a fanfiction URL to EPUB format with update detection
     * This method checks if the story was previously downloaded and handles updates
     */
    suspend fun convertFanfictionToEpubWithUpdateDetection(url: String): ConversionResult = withContext(Dispatchers.IO) {
        try {
            // Check if story exists
            val existingStory = updateManager.checkStoryExists(url)
            
            if (existingStory != null) {
                // Check for updates
                val updateCheck = updateManager.checkForUpdates(existingStory)
                
                if (updateCheck.hasUpdates && updateCheck.error == null) {
                    // Story has updates, re-download it
                    val updateResult = updateManager.updateStory(existingStory)
                    
                    return@withContext ConversionResult(
                        success = updateResult.success,
                        filePath = updateResult.updatedStory?.epubFilePath,
                        title = updateResult.updatedStory?.title,
                        author = updateResult.updatedStory?.author,
                        chapters = updateResult.updatedStory?.totalChapters ?: 0,
                        errorMessage = updateResult.error,
                        wasUpdate = true,
                        previousChapters = existingStory.lastKnownChapters,
                        storyId = existingStory.id
                    )
                } else if (updateCheck.error != null) {
                    return@withContext ConversionResult(
                        success = false,
                        errorMessage = "Update check failed: ${updateCheck.error}",
                        wasUpdate = false
                    )
                } else {
                    // No updates, return existing file info
                    return@withContext ConversionResult(
                        success = true,
                        filePath = existingStory.epubFilePath,
                        title = existingStory.title,
                        author = existingStory.author,
                        chapters = existingStory.totalChapters,
                        errorMessage = "Story is up to date (${existingStory.totalChapters} chapters)",
                        wasUpdate = false,
                        storyId = existingStory.id
                    )
                }
            } else {
                // New story, download it
                val result = convertFanfictionToEpub(url)
                
                if (result.success && result.filePath != null && result.title != null && result.author != null) {
                    // Track the new download
                    val (site, siteId) = extractSiteInfo(url)
                    val storyId = updateManager.trackDownloadedStory(
                        url = url,
                        title = result.title,
            ...
            ConversionResult(
                success = false,
                errorMessage = "Failed to convert fanfiction with update detection: ${e.message}",
                wasUpdate = false
            )
        }
    }
    ...
    /**
     * Extract site information from URL for tracking purposes
     */
    private fun extractSiteInfo(url: String): Pair<String, String> {
        return when {
            url.contains("archiveofourown.org") -> {
                val workId = Regex("works/(\\d+)").find(url)?.groupValues?.get(1) ?: "unknown"
                "ao3" to workId
            }
            url.contains("fanfiction.net") -> {
                val storyId = Regex("s/(\\d+)").find(url)?.groupValues?.get(1) ?: "unknown"
                "ffnet" to storyId
            }
            url.contains("wattpad.com") -> {
                ...
                "generic" to url.hashCode().toString()
            }
        }
    }
}