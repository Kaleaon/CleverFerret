...
class PlaybackStateManagerImpl(
    ...
    init {
            ...
            audiobookService.audiobookState.collect { state ->
                if (state.isPlaying || activePlayerType == "audiobook") {
                    val miniState = if (state.isPlaying) {
                        val chapterTitle = state.chapters.getOrNull(state.currentChapter)?.title
                        MiniPlayerState(
                            title = chapterTitle ?: state.title.ifEmpty { "Audiobook" },
                            subtitle = state.author.ifEmpty { state.audiobook?.author ?: "" },
                            artworkUrl = state.audiobook?.coverPath,
                            isPlaying = state.isPlaying,
        ...
        // Note: PodcastService doesn't expose a unified playback state flow
        // Podcast playback is handled by AdvancedPodcastPlayerService
        // which uses ExoPlayerService directly for the mini player
    }
    ...
    override fun playPause() {
        when (activePlayerType) {
            ...
            "audiobook" -> {
                // AudiobookService uses play/pause instead of togglePlayPause
                val state = audiobookService.audiobookState.value
                if (state.isPlaying) audiobookService.pause() else audiobookService.play()
            }
            // Podcast playback is managed through ExoPlayerService
        }
    }
    ...
    override fun skipNext() {
        when (activePlayerType) {
            ...
            "audiobook" -> {
                ...
                val state = audiobookService.audiobookState.value
                if (state.currentChapter < state.chapters.size - 1) {
                    audiobookService.goToChapter(state.currentChapter + 1)
                }
            }
            // Podcast skip handled by ExoPlayerService
        }
    }
